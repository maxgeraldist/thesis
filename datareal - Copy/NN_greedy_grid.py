# Import necessary modules
import pandas as pd
from sklearn.model_selection import train_test_split, GridSearchCV, KFold
from sklearn.preprocessing import StandardScaler
from sklearn.neural_network import MLPRegressor
from sklearn.metrics import mean_squared_error, r2_score
import matplotlib.pyplot as plt

# Load the data
df = pd.read_excel("neighborhood_final.xlsx")

# Define features and target
features = df.drop(["rent", "Index"], axis=1)
target = df["rent"]

# Split the data into training and test sets
X_train, X_test, y_train, y_test = train_test_split(
    features, target, test_size=0.3, random_state=0
)

# Standardize the features
scaler = StandardScaler()
X_train = scaler.fit_transform(X_train)
X_test = scaler.transform(X_test)

# Create a MLPRegressor object
mlp = MLPRegressor(random_state=0, max_iter=100000, early_stopping=True)

# Create a KFold object with shuffling
kf = KFold(n_splits=3, shuffle=True, random_state=0)

# Define the order of hyperparameters for tuning
hyperparameters = [
    {
        "learning_rate_init": [0.001, 0.01, 0.1],
        "hidden_layer_sizes": [
            (50),
            (100),
            (200),
            (500),
            (50, 50),
            (100, 100),
            (200, 200),
            (500, 500),
            (50, 50, 50),
            (100, 100, 100),
            (200, 200, 200),
            (500, 500, 500),
        ],
        "solver": ["adam", "lbfgs", "sgd"],
    },
    {"activation": ["relu", "tanh"]},
    {"alpha": [0.0001, 0.001, 0.01]},
    {"learning_rate": ["constant", "adaptive"]},
]

# Sequentially tune hyperparameters
for params in hyperparameters:
    grid_search = GridSearchCV(
        mlp,
        params,
        cv=kf,
        scoring="neg_mean_squared_error",
        n_jobs=-1,
        verbose=2,
        pre_dispatch=24,
    )
    grid_search.fit(X_train, y_train)
    best_params = grid_search.best_params_
    print(f"Best parameters: {best_params}")
    mlp.set_params(**best_params)

# If 'adam' is the best solver, tune its specific parameters
if mlp.solver == "adam":
    params = {
        "beta_1": [0.9, 0.8, 0.7],
        "beta_2": [0.999, 0.99, 0.9],
        "epsilon": [1e-08, 1e-07, 1e-06],
    }
    grid_search = GridSearchCV(
        mlp,
        params,
        cv=kf,
        scoring="neg_mean_squared_error",
        n_jobs=-1,
        verbose=2,
        pre_dispatch=24,
    )
    grid_search.fit(X_train, y_train)
    best_params = grid_search.best_params_
    print(f"Best parameters: {best_params}")
    mlp.set_params(**best_params)

# Now you can use the tuned model to make predictions
mlp.fit(X_train, y_train)
y_pred = mlp.predict(X_test)

# Calculate the mean squared error
mse = mean_squared_error(y_test, y_pred)
print(f"Mean Squared Error (MSE): {mse}")

# Calculate the R^2 score
r2 = r2_score(y_test, y_pred)
print(f"R^2 Score: {r2}")

with open("NN/neighborhood_NNTuned.txt", "w") as f:
    f.write(f"Mean Squared Error (MSE): {mse}\n")
    f.write(f"R^2 Score: {r2}\n")
    f.write(f"Best parameters: {best_params}\n")
    f.write(f"Predicted values: {y_pred}\n")
    f.write(f"Actual values: {y_test}\n")

hidden_layer_sizes = [(100), (500, 500), (500, 500, 500)]

# Manually fit the models and track the loss at each iteration
for hls in hidden_layer_sizes:
    mlp.set_params(hidden_layer_sizes=hls)
    losses = []
    for _ in range(10000):  # Set a large number of maximum iterations
        mlp.fit(X_train, y_train)
        losses.append(mean_squared_error(y_test, mlp.predict(X_test)))
        if mlp.n_iter_ == mlp.max_iter:  # The model has converged
            break
    plt.plot(losses, label=f"hidden_layer_sizes={hls}")
    plt.xlabel("Number of iterations")
    plt.ylabel("Mean Squared Error (MSE)")
    plt.legend()
    plt.show()
    plt.savefig(f"plot_{i}.png")  # Save the plot to a file
    plt.clf()  # Clear the current figure for the next plot

# Define the number of neurons for the single-layer regression
neurons = [50, 100, 200, 500]

# Initialize lists to store the results
neuron_counts = []
accuracies = []

# Run the regressions and track the accuracy
for n in neurons:
    mlp.set_params(hidden_layer_sizes=(n,))
    mlp.fit(X_train, y_train)
    accuracy = mlp.score(X_test, y_test)
    neuron_counts.append(n)
    accuracies.append(accuracy)

# Plot the results
plt.plot(neuron_counts, accuracies, "o-")
plt.xlabel("Number of Neurons")
plt.ylabel("Accuracy")
plt.title("Accuracy vs. Number of Neurons in Single-Layer Regression")
plt.grid(True)
plt.savefig("accuracy_vs_neurons.png")
plt.show()
